...
**BiliLiving** 把浏览、搜索、弹幕和手机投屏带到 Apple TV，重点改进 **面向美国等海外网络的高画质缓冲体验**，以及 **手机投送低清链接后的电视端高清取流**。打开即可游客观看，登录后获取账号推荐、访问收藏和观看历史，用遥控器就能完成日常操作。
...
| | 在电视上能做什么 |
| :--- | :--- |
| 🌎 **海外观看，更长缓冲** | 针对美国等海外用户访问 B 站 CDN 时的网络波动，新增 30 秒／2 分钟／5 分钟前向缓冲目标，默认 2 分钟，为 4K 等高码率视频提供更多缓冲余量。 |
| 📺 **手机投低清，电视取高清** | 手机仅投送普通 DLNA 720p 链接时，尝试识别原视频，由已登录的电视重新获取可用高清流，恢复清晰度菜单和弹幕。 |
...
### 面向美国等海外网络的 4K／高码率播放改进

上游已有 4K 播放、CDN 测速与卡顿换源能力。BiliLiving 在此基础上，将普通视频连续观看的前向缓冲目标从固定 15 秒扩展为 **30 秒、2 分钟或 5 分钟**，默认 **2 分钟**。更长的缓冲旨在帮助美国等海外用户抵抗跨境链路的短时吞吐波动，减少高码率播放中反复等待缓冲的情况。

- 在播放器 **播放设置 → 视频预缓冲** 中即时调整；网络波动较大时可尝试 5 分钟。
- 起播和拖动进度时暂用较短缓冲，稳定后恢复长缓冲；无需缓存满 2／5 分钟才开始播放。
- 新增“最佳可用”默认画质，并按所选档位生成播放列表，减少已选择 4K 后被播放器自动降到低档位的情况。

缓冲时长是交给 AVPlayer 的目标，实际缓存量由系统与网络决定；长期带宽不足时仍可能卡顿。实现和验证边界见 [缓冲策略](docs/PLAYBACK-BUFFERING.md)。

### 从手机低清投屏链接恢复电视端高清播放

上游已有携带视频标识的投屏取流能力。BiliLiving 新增的处理覆盖另一种情况：**手机只发送普通 DLNA 媒体链接，没有 AV／BV 标识**。电视从可信 B 站链接中提取 CID，结合标题查找候选，并通过 B 站分 P 信息精确校验 CID；匹配成功且电视已登录时，切换到原生播放器重新取流。

因此，即使手机投屏控制页只显示 720p，电视也可以使用自身账号获取服务端允许的 1080p／4K 等档位，并恢复弹幕和播放设置。**实际画质取决于电视账号权限、视频源和服务端返回结果，不会绕过会员权限。** 游客或识别失败时保留手机原始链接，避免匹配到错误视频或降低已有流的画质。详见 [投屏说明](docs/CASTING.md)。
